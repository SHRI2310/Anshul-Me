...
//  return res.send(findQuote)
  let url ="https://sandbox-in-gw.insuremo.com/riabroker/1.0/broker-bff-app/v1/redirect"
   ...
   let axiosCall = await axios.post (url,data1,config)
    res.status(200).cookie("bearer",axios1?.data?.access_token).send(axiosCall.data)
 ...
 export  const statusCheck = tryCatch(async(req,res,next)=>{
 ...
 //  console.log(findQuote)
 //  return res.send(findQuote)
   let url ="https://sandbox-in-gw.insuremo.com/riabroker/1.0/broker-bff-app/v1/statusCheck"
    ...
    // console.log(axiosCall)
     res.send({Status:axiosCall.data,Data:findQuote})
